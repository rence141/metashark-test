-- Add only the missing columns to your existing users table
-- Based on your current table structure

-- Add role column (most important)
...
-- Add foreign key constraint for seller_id (if it doesn't exist)
-- This might show an error if constraint already exists, but that's okay
